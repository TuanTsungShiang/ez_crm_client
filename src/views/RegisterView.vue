<script setup lang="ts">
  ...
  import { getRegisterSchema, register, type RegisterPayload } from '@/api/auth'
...
<template>
  <section class="mx-auto max-w-md">
    ...
    <!-- 載入錯誤 -->
    <p v-else-if="loadError" class="mt-6 text-sm text-red-500">無法載入表單:{{ loadError }}</p>

    <!-- 表單 -->
    <div v-else-if="schema" class="mt-6 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
      <p v-if="topLevelError" class="mb-4 rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">
        {{ topLevelError }}
      </p>
      ...
      <p class="mt-4 text-xs text-slate-400">
        ...
        <a :href="schema.links.terms" target="_blank" class="underline hover:text-slate-600">
          服務條款
        </a>
        ·
        <a :href="schema.links.privacy" target="_blank" class="underline hover:text-slate-600">
          隱私權
        </a>
      </p>

      <div class="mt-6 border-t border-slate-200 pt-6">
        <OAuthButtons v-if="schema.oauth_providers.length > 0" mode="register" />
      </div>
    </div>
  </section>
</template>
