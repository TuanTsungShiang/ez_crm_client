...
<template>
  ...
  <section v-else class="space-y-6">
    <!-- Hero greeting -->
    <div
      class="rounded-xl bg-gradient-to-br from-blue-500 via-indigo-500 to-purple-600 p-8 text-white shadow-lg"
    >
      <p class="text-sm opacity-80">{{ greeting }} 👋</p>
      <h1 class="mt-1 text-3xl font-bold">
        {{ auth.member?.name }}
      </h1>
      <p class="mt-2 text-sm opacity-90">歡迎回到 ez_crm,今天也要好好照顧你的會員資料。</p>
    </div>
    ...
    >
      ...
      <dl class="mt-3 space-y-1 font-mono">
        ...
        <div class="break-all"><span class="text-slate-400">token:</span> {{ auth.token }}</div>
      </dl>
    </details>
  </section>
</template>
